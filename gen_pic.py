    ...
    if type_long == 0:
        ...
        if block_flag <= 3:                              # 有30%的概率不产生空格
            ...
            for j in range(len(space_flag_np)):                        # 按照位置随机插入随机长度的空格
                ...
                random_word = random_word[0:space_set_flag] + " " * space_num + random_word[space_set_flag: len(random_word)]
                # 插入空格
...
def random_background(bground_path):                 # 随机选取背景图片
    ...
    if random.randint(0, 9) < 7:
        ...
        perspective[2][2] = 1.0
        bground = cv2.warpPerspective(bground, perspective, (bground.shape[1], bground.shape[0]), borderValue=(255, 255, 255))
...
def rec_cross(box1, box2):
    ...
    for i in range(4):
        ...
        if line_cross(x1_1, y1_1, x2_1, y2_1, box2[i][0], box2[i][1], box2[(i + 1) % 4][0], box2[(i + 1) % 4][1])\
            or line_cross(x2_1, y2_1, x3_1, y3_1, box2[i][0], box2[i][1], box2[(i + 1) % 4][0], box2[(i + 1) % 4][1])\
            or line_cross(x3_1, y3_1, x4_1, y4_1, box2[i][0], box2[i][1], box2[(i + 1) % 4][0], box2[(i + 1) % 4][1])\
            or line_cross(x4_1, y4_1, x1_1, y1_1, box2[i][0], box2[i][1], box2[(i + 1) % 4][0], box2[(i + 1) % 4][1]):
            cross_flag = 1
            break
...
def main(num, nclass, dict):
    background = random_background('./background/')

    str_num = random.randint(3, 10)
    # print(str_num)
    ...
    for i in range(str_num):
        ...
        '''b_channel, g_channel, r_channel = cv2.split(imgRotation)
        alpha_channel = np.ones(b_channel.shape, dtype=b_channel.dtype)
        channel_sum = b_channel + g_channel + r_channel
        alpha_channel = alpha_channel * channel_sum
        alpha_channel[alpha_channel > 0] = 255
        img_BGRA = cv2.merge((b_channel, g_channel, r_channel, alpha_channel))
        img_show(img_BGRA, 0)
        cv2.imwrite('./data_set/img/img_NUM_{}.png'.format(num), img_BGRA)'''


        for k in range(0, len(boxes)):
            [[x1_1, y1_1], [x2_1, y2_1], [x3_1, y3_1], [x4_1, y4_1]] = boxes[k]

            if random_word[k] == " ":
                continue

            boxses.append([x1_1, y1_1, x2_1, y2_1, x3_1, y3_1, x4_1, y4_1])
            labels.append(dataset_unit.str_Converter(random_word[k], dict, nclass))
            
            # cv2.drawContours(background, [boxes[k]], 0, (0, 0, 255), 3)
        # cv2.drawContours(background, [box_rec], 0, (255, 0, 0), 2)
        # img_show(background, 300)
    ...
    cv2.imwrite('./data_set/img/img_{}.jpg'.format(num), background)
    bboxes = np.array(boxses)
    tags = np.array(labels)
    img, bboxes = dataset.resize_img(background, bboxes)

    geo_map, conf_map, tag_map = dataset.get_score_geo(img, bboxes, tags, nclass, scale=0.25, length=512)
    save_np = np.concatenate((geo_map, conf_map, tag_map), 2)
    print(save_np.shape)
    np.save('./data_set/gt_np/img_{}.npy'.format(num), save_np)
...
if __name__ == '__main__':
    ...
    num_th = args.th_num
    start_num = num_th * once_total
    end_num = (num_th + 1) * once_total
